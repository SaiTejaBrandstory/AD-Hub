...
import { Pause, Play, DotsThree, FunnelSimple } from "@phosphor-icons/react";
            ...
            {loading ? (
              ...
              <tr key={c.campaign_id} className="border-b border-ink-200 last:border-0 hover:bg-ink-50/60 transition-colors">
                <td className="px-4 py-3">
                  <Link to={`/campaigns/${c.campaign_id}`} className="font-medium text-ink hover:underline" data-testid={`campaign-link-${c.campaign_id}`}>
                    {c.name}
                  </Link>
                ...
                <td className="px-4 py-3 text-right font-num text-ink-400">{(c.impressions/1000).toFixed(1)}k</td>
                ...
                <td className="px-4 py-3 text-right font-num">${c.cpa}</td>
    ...
    <div className="inline-flex items-center gap-1 bg-white border border-ink-200 rounded-lg p-0.5">
      ...
      {options.map(([v, l]) => (
        <button
          key={v}
          onClick={() => onChange(v)}
          data-testid={`filter-${label.toLowerCase()}-${v}`}
